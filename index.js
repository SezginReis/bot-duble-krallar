...
.setThumbnail()
.addField(`Duble Krallar | Teşekkürler`, `**Selamlar, Ben Sezgin (Duble Krallar'ın Geliştiricisi) Öncelikle Botumuzu Eklediğiniz ve Bize Destek Olduğunuz İçin Sizlere Teşekkürlerimi Sunarım**`)
.addField(`Alone | Prefix`, `**Duble Krallar Botun Prefixi(ön eki) = \`${fynx.prefix}\`\n\n Değiştirebilmek için \`${fynx.prefix}prefix\` Yazabilirsiniz.**`)
.addField(`Duble Krallar | Nasıl Kullanılır?`, `**Duble Krallar botun tüm özelliklerinden yararlanabilmek için sadece \`${fynx.prefix}yardım\` yazmanız yeterlidir.**`)
.addField(`Duble Krallar | Linkler`, `**Sohbet Kanalına d!davet Yazmanız Yeterlidir**`)
.setFooter(`Duble Krallar | Gelişmiş Türkçe Bot | 2020`)
.setTimestamp();
...
var oyun = [
`「☪」Duble Krallar | d!yardım`,
`「🚀」 Yardım Almak İçin | d!yardım`,
`「🔔」 Yenilenen Tasarımı İle`,
`「⚡️」Botu eklemek için | d!davet`,
`「🌟」Prefix ayarlamak için | d!prefix`
]
...
client.on('message', async msg => {
  ...
  if(msg.content == `<@!713713727794446397>`) return msg.channel.send(`> **Duble Krallar | Prefix**\n\n> <a:ayarlar1:751520036149264454> **Sanırım beni etiketlediniz.**\n > <a:hypesquad1:750076071721828452> Buyurun prefix(ön ek)im \`${prefix}\``);
});
...
.setDescription(`<a:ayarlar1:751520036149264454> <@!${message.author.id}> **adlı kullanıcı tarafından** <#${message.channel.id}> **kanalına gönderilen mesaj silindi!** \n\nSilinen Mesaj: **${message.content}**`)
  .setFooter("Duble Krallar | Log Sistemi")
...
if (!modlogkanal) return;
  ...
  .setColor("#ffd100")
    ...
    .setDescription(`<a:ayarlar1:751520036149264454> **Üye Sunucudan Yasaklandı!** \n<@!${message.user.id}>, ${message.user.tag}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                ...
                .setDescription(`<a:ayarlar1:751520036149264454> ${channel.name} **Adlı Metin Kanalı Oluşturuldu!**`)
                .setFooter(`Duble Krallar Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
...
.setTitle("SES KANALI OLUŞTURULDU")
                .setDescription(`<a:ayarlar1:751520036149264454> ${channel.name} **Adlı Ses Kanalı Oluşturuldu!**`)
                .setFooter(`Duble Krallar | Log Sistemi Kanal ID: ${channel.id}`)
...
if (!modlogkanal) return;
    if (channel.type === "text") {
                let embed = new Discord.MessageEmbed()
                    .setColor('#ffd100')
                .setDescription(`<a:ayarlar1:751520036149264454> ${channel.name} **Adlın Metin Kanalı  Silindi**`)
                .setFooter(`Duble Krallar Bot | Log Sistemi Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            };
            if (channel.type === "voice") {
...
.setTitle("SES KANALI SİLİNDİ")
                .setDescription(`<a:ayarlar1:751520036149264454> ${channel.name} **Adlı Ses Kanalı Silindi**`)
            .setFooter(`Duble Krallar Bot | Log Sistemi  Kanal ID: ${channel.id}`)
                modlogkanal.send({embed});
            }
    })
    ...
    if (küfür.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`<a:ayarlar1:751520036149264454> **Hey ${message.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`<a:ayarlar1:751520036149264454> **Hey ${newMessage.author} Dur! Bu Sunucuda Küfürü Engelliyorum!**`).then(message => message.delete(3000));
...
client.on('guildMemberAdd', async(member) => {
   ...
   let channel = client.channels.get("756181804435898484")//KANAL ID
    ...
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('BAN_MEMBERS')) {
          ...
          return message.channel.send(`<a:ayarlar1:751520036149264454> **Hey ${message.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
      ...
      try {
        if (!newMessage.member.permissions.has('BAN_MEMBERS')) {
         newMessage.delete();
          ...
          return newMessage.channel.send(`<a:ayarlar1:751520036149264454> **Hey ${newMessage.author} Dur! Bu Sunucuda Reklamı Engelliyorum!**`).then(message => message.delete(3000));
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'sa' || msg.content.toLowerCase() == 's.a' || msg.content.toLowerCase() == 'selamun aleyküm' || msg.content.toLowerCase() == 'sea' || msg.content.toLowerCase() == 's.a.' || msg.content.toLowerCase() == 'selam' || msg.content.toLowerCase() == 'slm') {
          try {
                  ...
                  return msg.reply('<a:kral2:740243285376761916> **Aleyküm Selam, Hoşgeldin** <a:yldz:752666455337861250>')
          } catch(err) {
            console.log(err);
          }
      }
...
client.on("guildMemberAdd", async member => {
  ...
  ///....
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:ayarlar1:751520036149264454> `"+ member.user.username + "`**Adlı Kullanıcı Aramıza Katıldı!** `" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
  }

  if (member.guild.memberCount == sayaç) {
    return client.channels.get(kanal).send(`<a:ayarlar1:751520036149264454> **Sayaç Sıfırlandı!** \`${member.guild.memberCount}\` **Kişiyiz!**`)
...
client.on("guildMemberRemove", async member => {
  ...
  if (!mesaj) {
    return client.channels.cache.get(kanal).send("<a:ayarlar1:751520036149264454> `" + member.user.username + "` **Adlı Kullanıcı Aramızdan Ayrıldı.**`" + sayaç + "` **Kişi Olmamıza** `" + sonuç + "` **Kişi Kaldı.** `" + member.guild.memberCount + "` **Kişiyiz!**");
      }
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache.get(kanal).send("<a:ayarlar1:751520036149264454> **Selam!** `" + member.user.username + "`**!kayıtol yazarak kayıt olabilirsin!**");
...
client.on("guildMemberAdd", async member => {
  ...
  if (!mesaj) {
    client.channels.cache.get(kanal).send("<a:ayarlar1:751520036149264454> `" + member.user.username + "`** Hoş Geldin! Otomatik Rolün Verildi Seninle Beraber** `" + member.guild.memberCount + "` **Kişiyiz!**");
