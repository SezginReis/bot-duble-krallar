...
if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send(`<a:ayarlar1:751520036149264454>  **Bu komutu kullanabilmek için** "\`Yönetici\`" **yetkisine sahip olmalısın.**`);
 ...
 if(!rol) return message.channel.send(`<a:ayarlar1:751520036149264454>  **Kayıt Tamamlandığı Zaman Alınacak Rolü Ayarlamak İçin Bir Rol Etiketlemelisiniz!**\n**__Örnek__**: \`${prefix}kayıt-alınacak-rol-ayarla @AlınacakRol\``)
 
  message.channel.send(`<a:ayarlar1:751520036149264454>  **Kayıt Olan Kullanıcılardan Alınacak Otomatik Rol** \`${rol}\` **Şeklinde Ayarlandı!**`)
