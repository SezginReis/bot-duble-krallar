...
.setDescription(`<a:emoji_200:740256355545383032>   **| Müzik oynatabilmek için bir ses kanalında olmanız gerekmektedir!**`)
...
.setDescription(`<a:emoji_200:740256355545383032>   **| Müzik oynatabilmek için aranacak kelime veyahut kelimeler giriniz!**`)
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());    
...
const sarki = await client.player.addToQueue(message.guild.id, args.join(" "));
  ...
  .setDescription(`<a:emoji_200:740256355545383032>  **|** \`${sarki.name}\` **adlı müzik,**\n${message.author} **tarafından kuyruğa eklendi!**`)
.setThumbnail(`https://drummofficial.com/wp-content/uploads/2017/08/equalizer3.gif`)
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());   
...
const sarki = await client.player.play(message.member.voice.channel, args.join(" "));
  ...
  .setColor(AloneDogru)
  .setTitle("Duble Krallar | Oynatılan Şarkı")
  .setDescription(`<a:emoji_200:740256355545383032>  **|** \`${sarki.name}\` **adlı müzik şu anda oynatılıyor.**`)
...
sarki.queue.on('end', () => {
  ...
  .setColor(AloneDogru)
  .setTitle("Duble Krallar | Kuyruk Bitti!")
  .setDescription(`<a:emoji_200:740256355545383032>  **| Kuyruktaki tüm müzikler oynatıldı. Duble Krallar kanaldan ayrılıyor.\n\nDuble Krallar Tercih ettiğiniz için teşekkür ederiz.**`)
  .setThumbnail(client.user.avatarURL())
.setFooter(`©️ Tüm hakları saklıdır | Yeni Nesil Gelişmiş Bot | 2020`, client.user.avatarURL());  
